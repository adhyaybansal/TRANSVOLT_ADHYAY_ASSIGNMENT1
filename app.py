...
def generate_charts():
    ...
    df.iloc[peaks].to_csv("static/local_peaks.csv", index=False)
    df.iloc[lows].to_csv("static/local_lows.csv", index=False)

    
    df[df['Values'] < 20].to_csv("static/voltage_below_20.csv", index=False)

   
    df['Slope'] = df['Values'].diff()
    df[(df['Slope'] < 0) & (df['Slope'].diff() < 0)][['Timestamp', 'Values', 'Slope']] \
        .to_csv("static/downward_accel.csv", index=False)
...
def index():
    ...
    return render_template("index.html", interpretations=interpretations)
